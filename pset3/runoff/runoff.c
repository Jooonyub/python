...
int main(int argc, string argv[])
{
    ...
    for (int i = 0; i < candidate_count; i++)       //candidates라는 'candidate'자료형 초기화
    {
...
{
    int count_getmin = 0;
    int count_currentcandidates = 0;                        //최저득표자의 수를 파악키 위한 변수.(0으로 초기화)
    for (int j = 0; j < candidate_count; j++)
    {
        if (candidates[j].eliminated == false)
        {
            count_currentcandidates++;
            if (candidates[j].votes == min)                 //각 후보마다 최저득표수와 비교
            {
                count_getmin++;                             //해당 후보의 득표수 = 최저득표수 이면 count값을 + 1
            }
        }
    }
    if (count_getmin == count_currentcandidates)
    {
        return true;
    }
    else
