...
/*
    ...
    while (true)
    {
        // Calculate votes given remaining candidates       표 집계
        tabulate();

        // Check if election has been won                   결과 확인
        ...
        // Eliminate last-place candidates                  결과 안났으면 최저득표 제외
        int min = find_min();
        bool tie = is_tie(min);

        // If tie, everyone wins                            모두가 동률일경우 모두 당선
        if (tie)
        {
        ...
        // Reset vote counts back to zero                   득표 집계 리셋
        for (int i = 0; i < candidate_count; i++)
        {
...
//X
// Tabulate votes for non-eliminated candidates - 문제점 : 각 투표자의 최우선 preference만 집계되어야하는데, 모든 루프가 다 돌고 있다.
void tabulate(void)
{
    for (int i = 0; i < voter_count; i++)
    {
        ...
        {
            printf("Since the candidate of your 1st preference is eliminated, your vote for the secondary candidate(%s) counts.  \n", candidates[preferences[i][j+1]].name);
            candidates[preferences[i][j+1]].votes++;
            printf("%i th voter's %i th preference candidate vote : %s\n", i+1, j+2, candidates[preferences[i][j+1]].name);
        }
        else
        {
            printf("Your vote is counted for your 1st preference(%s)\n", candidates[preferences[i][j]].name);
...
int find_min(void)
{
    ...
    printf("The minimum votes are : %i\n", min);
    // TODO
    return 0;
}
